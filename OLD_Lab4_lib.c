...
int concatenateStrings(char* str1, char* str2)
{
  ...
  char tempString[firstArrayLength+secondArrayLength];

  if (tempString == 0)
  {
    printf("There was an error expanding array\n");
    return 1;
  }
  ...
  for (i=0; i<(firstArrayLength+secondArrayLength); i++)
  {
    ...
    {
      tempString[i] = str2[i-firstArrayLength];
    }
  }
  str1 = malloc(sizeof(char) * (firstArrayLength+secondArrayLength));
  i=0;
  while(i<firstArrayLength+secondArrayLength)
  {
    printf("STRING - %c", tempString[i]);
    str1[i] = tempString[i];
    i++;
  }
  str1[i] = '\0';
